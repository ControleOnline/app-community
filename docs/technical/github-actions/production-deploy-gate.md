...
O gate existe para impedir publicação em produção (`master`) sem verificações locais reproduzíveis. A publicação não aguarda um deploy em `staging`: staging é uma superfície de conferência humana, não uma dependência técnica do deploy de produção.
...
O job roda somente quando o ambiente alvo é `master` e executa no commit que será publicado: instalação sem scripts, verificação de sintaxe JavaScript, validação do catálogo de smoke flows, suíte Jest em modo serial e `git diff --check`. Se qualquer verificação falhar, a publicação para; se todas passarem, não é necessário aguardar `staging`.
